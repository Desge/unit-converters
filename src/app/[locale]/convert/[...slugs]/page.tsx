...
const ConverterTool = dynamic(() => import('@/components/ConverterTool'), {
  loading: () => (
    <div className="max-w-3xl mx-auto px-4 py-8">
      <div className="card p-6 animate-pulse space-y-4">
        <div className="h-10 bg-[var(--bg-tertiary)] rounded-lg w-1/3" />
...
export default function ConverterPage({ params }: { params: { locale: string; slugs: string[] } }) {
  ...
  return (
    <>
      {/* SEO Structured Data */}
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(generateWebAppSchema(pair, locale)) }} />
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(generateBreadcrumbSchema(pair, locale)) }} />
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(generateFAQSchema(pair, locale)) }} />
      ...
      <div className="max-w-3xl mx-auto px-4 pt-6">
        <nav className="flex items-center gap-2 text-sm text-[var(--text-tertiary)]" aria-label="Breadcrumb">
          <a href={`/${locale}/`} className="hover:text-brand-500 transition-colors">{t('home', locale)}</a>
          <span>/</span>
          <a href={`/${locale}/#${pair.categoryKey}`} className="hover:text-brand-500 transition-colors">{catName}</a>
          <span>/</span>
          <span className="text-[var(--text-primary)] font-medium truncate max-w-[200px]">{fromName} → {toName}</span>
        </nav>
      </div>
      ...
      <div className="max-w-3xl mx-auto px-4 pb-16">
        <div className="card p-6">
          <h3 className="font-semibold text-lg mb-4">
            {t('more', locale)} {catName} {t('conversions', locale)}
          </h3>
